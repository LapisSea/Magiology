	...
	public void onUpdate(){
		...
		if(type==1){
			...
			if(particleAge<particleMaxAge/8.5/2){
				...
				if(U.getMC().gameSettings.particleSetting==0&&worldObj.rand.nextBoolean())UtilM.spawnEntityFX(new EntitySmoothBubleFX(worldObj,posX+x1*4, posY+y1*4, posZ+z1*4, -x1/3+xSpeed,-y1/3+ySpeed, -z1/3+zSpeed, particleMaxAge/2, 3,0, false,1,"tx1",r_e,g_e,b_e, 1.0, 0.99));
			}
			if(particleAge>particleMaxAge/8.5/2){
				particleScale-=0.004*particleMaxAge;
				if(active==true&&U.getMC().gameSettings.particleSetting==0&&worldObj.rand.nextBoolean())UtilM.spawnEntityFX(new EntitySmoothBubleFX(worldObj,posX, posY, posZ, 0.025-0.05*worldObj.rand.nextFloat()+xSpeed,0.025-0.05*worldObj.rand.nextFloat()+ySpeed, 0.025-0.05*worldObj.rand.nextFloat()+zSpeed, particleMaxAge/2, 3,-10, false,1,"tx1",r_e,g_e,b_e, 1.0, 0.99));
			}
			if(particleAge>particleMaxAge/8.5){
				setExpired();
				if(active==true){
					for(int t=0;t<2;t++)UtilM.spawnEntityFX(new EntitySmoothBubleFX(worldObj,posX, posY, posZ,0.025-0.05*worldObj.rand.nextFloat()+xSpeed,0.025-0.05*worldObj.rand.nextFloat()+ySpeed, 0.025-0.05*worldObj.rand.nextFloat()+zSpeed, particleMaxAge/2, 4,-1, false,1,"tx1",r_e,g_e,b_e, opacity_e, 0.99));
				}
			}
		}
		else if(type==3){
			...
			if(particleAge<particleMaxAge/8.5/3){
				...
				setExpired();
					for(int t=0;t<40;t++)UtilM.spawnEntityFX(new EntitySmoothBubleFX(worldObj,posX, posY, posZ,1-2*worldObj.rand.nextFloat()+xSpeed,3-4*worldObj.rand.nextFloat()+ySpeed*2, 1-2*worldObj.rand.nextFloat()+zSpeed, particleMaxAge*4, 6,-1, false,1,"tx1",worldObj.rand.nextFloat(),worldObj.rand.nextFloat(),worldObj.rand.nextFloat(), 1, 0.99));
			}
		}
		else if(type==6){
//			motionY-=0.001;
			if(particleAge<particleMaxAge/8.5/3){
				...
				else UtilM.spawnEntityFX(new EntitySmoothBubleFX(worldObj,posX, posY, posZ,1-2*worldObj.rand.nextFloat()+xSpeed,1-2*worldObj.rand.nextFloat()+ySpeed, 1-2*worldObj.rand.nextFloat()+zSpeed, particleMaxAge*2, 4,-20, false,1,"tx1",r_e,g_e,b_e, opacity_e, 0.99));
			}
		}
		else if(type==7){
			if(particleAge>particleMaxAge/8.5){
	...
	public void render(){
		...
		if(!hasDepth)GL11U.glDepth(false);
		...
		if(!hasDepth)GL11U.glDepth(true);
