...
public class RealPhysicsMesh{
	...
	public static enum MaterialStrategy{
		NO_INTERACTION(new ObjectProcessor<Boolean>(){

			@Override
			public Boolean process(Boolean object, Object... objects){return false;}
		}, false),
		ONLY_BIGGER_SUPPRESSING(new ObjectProcessor<Boolean>(){

			@Override
			public Boolean process(Boolean object, Object... objects){return (Float)objects[0]>0;}
		}, true),
		ONLY_SMALLER_SUPPRESSING(new ObjectProcessor<Boolean>(){

			@Override
			public Boolean process(Boolean object, Object... objects){return (Float)objects[0]<0;}
		}, true),
		STATIC_DISTANCE(new ObjectProcessor<Boolean>(){

			@Override
			public Boolean process(Boolean object, Object... objects){return true;}
		}, true);
	...
	public static PairM<List<Vec3M>,List<Vec2i>> create2DPlane(int xCubes, int yCubes, float xSize, float ySize){
		...
		for(int i=0; i<yCubes; i++){
			for(int j=0; j<xCubes; j++){
				int pos1=0, pos2=1, pos3=xPoints, pos4=xPoints+1;
	...
	public void update(){
		...
		if(interactStrategy.shouldCheck){
			...
			for(int pos=0; pos<indices.size(); pos++){
				...
				boolean xHooked=isVertexHooked(con.x), yHooked=isVertexHooked(con.y);
				if(!xHooked||!yHooked){
					AbstractRealPhysicsVec3F vertex1=vertices.get(con.x), vertex2=vertices.get(con.y);
					...
					if(interactStrategy.shouldInteract(difference)){
						...
						float mul1=xHooked?0:yHooked?2:1, mul2=xHooked?-2:yHooked?0:-1;
