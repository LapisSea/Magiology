...
public class AngularVec3{
	...
	public AngularVec3(Vec3M vec){
		float distanceX=-vec.getX(), distanceY=-vec.getY(), distanceZ=-vec.getZ(), rotationX=(float)-Math.toDegrees(Math.atan2(distanceY, new Vec3M(-distanceX, 0, -distanceZ).lengthVector())), rotationY=(float)-Math.toDegrees(Math.atan2(distanceX, -distanceZ));
	...
	public float angleToDot(){
		float angleArea=180, x=Math.abs(getXRotation()), y=Math.abs(getYRotation());
