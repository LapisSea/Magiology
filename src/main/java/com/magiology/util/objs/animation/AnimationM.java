...
public class AnimationM{
	...
	private PairM<LinearAnimation<CalculableFloat>[],String[]> parse(String string){
		...
		for(String line : lines){
			...
			Vec2FM l1p1=new Vec2FM(start.pos, start.value.value), l2p1=new Vec2FM(end.pos, end.value.value);
			
			float diffx=l1p1.x+1-l2p1.x, diffy=l1p1.y-l2p1.y, angle=(float)Math.atan2(diffx, diffy), sin=MathHelper.sin(angle), cos=MathHelper.cos(angle);
