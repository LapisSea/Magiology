...
public class QuadUV{
	...
	public Vec2FM[] toArray(){
		return new Vec2FM[]{new Vec2FM(x1, y1), new Vec2FM(x2, y2), new Vec2FM(x3, y3), new Vec2FM(x4, y4)};
	}
	...
	public QuadUV translate(int id, float x, float y){
		switch(id){
		case 0:
			return new QuadUV(x1+x, y1+y, x2, y2, x3, y3, x4, y4);
		case 1:
			return new QuadUV(x1, y1, x2+x, y2+y, x3, y3, x4, y4);
		case 2:
			return new QuadUV(x1, y1, x2, y2, x3+x, y3+y, x4, y4);
		case 3:
			return new QuadUV(x1, y1, x2, y2, x3, y3, x4+x, y4+y);
		}
		return this;
	}
}
