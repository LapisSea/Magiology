...
public class PartialTicksUtil{
	...
	public static Vec3M calculate(Entity entity){
		return new Vec3M(calculateX(entity), calculateY(entity), calculateZ(entity));
	}
	...
	public static Vec3M calculate(Vec3M prevPos, Vec3M pos){
		return new Vec3M(calculate(prevPos.x(), pos.x()), calculate(prevPos.y(), pos.y()), calculate(prevPos.z(), pos.z()));
	}
	
	public static Vector2f calculate(Vector2f prevVec, Vector2f vec){
		return new Vector2f(calculate(prevVec.x, vec.x), calculate(prevVec.y, vec.y));
	}
	...
	public static ColorM calculate(ColorM prevColor, ColorM color){
		return new ColorM(calculate(prevColor.r(), color.r()), calculate(prevColor.g(), color.g()), calculate(prevColor.b(), color.b()), calculate(prevColor.a(), color.a()));
	}
