...
public class ItemRendererTheHand implements CustomRenderedItemRenderer{
	private static HandData lastData=null;
	private final static float p=1F/16F;
	
	private static Matrix4f worldMat;
	
	public static Vec3M getPalmMiddle(){
		Vec3M vec1=getVertexInWorld(0);
		Vec3M vec2=getVertexInWorld(7);
		return vec1.add(vec2).mul(0.5);
	}
	
	public static Vec3M getPalmRand(){
		Vec3M 
			vec1=getVertexInWorld(0),
			vec2=getVertexInWorld(3),
			vec3=getVertexInWorld(4),
			xDist=vec3.sub(vec1),
			zDist=vec2.sub(vec1),
			xOffset=xDist.mul(RandUtil.RF()),
			zOffset=zDist.mul(RandUtil.RF());
		
		
		
		return vec1.add(xOffset).add(zOffset);
	}
	public static Vec3M getVertexInWorld(int id){
		if(lastData==null)return new Vec3M();
		if(worldMat==null){
			Matrix4f
			handm=MatrixUtil
				.createMatrixX(0)
				.translate(p*13, p*3, 0)
				.rotate(new Vector3f(0, 120, 0))
				.translate(lastData.base[0]+p*3, lastData.base[1]+p*4, lastData.base[2]-p*12)
				.translate(-p,0,-p)
				.rotateAt(new Vector3f(p*4,0,0),new Vector3f(lastData.base[3], lastData.base[4], lastData.base[5]))
				.finish(),
			itemM=TessUtil.getItemPos();
			worldMat=Matrix4f.mul(itemM,handm,null);
		}
		
		return TheHandHandler.getRenderer().handModelSolid.rednerLeftover.get(id).transform(worldMat);
	}
	...
	public MultiTransfromModel handModelSolid,handModelOpaque,handModelGlow;
	...
	private void generateModel(HandData data){
		...
		if(handModelSolid==null){
			...
			for(int i=0;i<2;i++){
				...
				child.addCube(cub);
				model=i==0?handModelSolid:handModelGlow;
				...
				child.addCube(cub3);
//				cub3.translate(p,0,0);
				child.addCube(cub3);
//				cub3.translate(0,0,p);
				child.addCube(cub3);
//				cub3.translate(-p,0,0);
				child.addCube(cub3);
//				cub3.translate(0,p,0);
				child.addCube(cub3);
//				cub3.translate(p,0,0);
				child.addCube(cub3);
//				cub3.translate(0,0,-p);
...
//				cub3.translate(-p,0,0);
		...
		handModelGlow.cell=new GlStateCell(new GlState(()->{
			...
			OpenGLM.color(1, 1, 1, data.cubeGlowPrecentage);
		...
		handModelOpaque.cell.willRender=()->data.calciferiumPrecentage>1F/256;
...
//		if(!(a1[9]+"").contains("com.magiology.forgepowered.events.client.RenderEvents"))return;
//        GlStateManager.depthMask(true);
//		for(int i=0;i<TheHandHandler.getRenderer().handModelSolid.rednerLeftover.size();i++){
//			Vec3M pos1=ItemRendererTheHand.getVertexInWorld(i);
//			EntitySmoothBubleFX part=new EntitySmoothBubleFX(UtilM.getTheWorld(),pos1.x,pos1.y,pos1.z,0,0,0 ,30,0.6F,0,1,0.2,0,1);
//			part.hasDepth=false;
...
//		}
		try{
			...
			if(data.cubes==null){
				transformationsCube.add(new Matrix4f());
				transformationsCube.add(new Matrix4f());
				transformationsCube.add(new Matrix4f());
				transformationsCube.add(new Matrix4f());
				transformationsCube.add(new Matrix4f());
				...
				for(DoubleObject<PhysicsVec3F,PhysicsVec3F> cube:data.cubes){
			...
			GL11U.glRotate(0, 120, 0);
			OpenGLM.translate(data.base[0]+p*3, data.base[1]+p*4, data.base[2]-p*12);
			if(data.calciferiumPrecentage>0.7){
				float noise=(data.calciferiumPrecentage-0.7F)/40;
	...
	public void renderItem(ItemStack stack, TransformType position){
		...
		if(position!=TransformType.FIRST_PERSON){
			...
			case GUI:{
				OpenGLM.translate(p*12,-p*10,-1.1224111F);
				GL11U.glRotate(20,-120,0,p*4,p,0);
			}break;
			case THIRD_PERSON:{
				GL11U.glScale(p*6);
