...
public class SoftEffectsShader extends PositionAwareEffect{
	...
	protected UniformUploaderColor[] cricle_mulColor   =new UniformUploaderColor[20];
	protected UniformUploaderF2      cricle_screenPos[]=new UniformUploaderF2[20];
	protected UniformUploaderF1 cricle_sizeP[]=new UniformUploaderF1[20], cricle_sizeN[]=new UniformUploaderF1[20], cricle_radius[]=new UniformUploaderF1[20], cricleArraySize;
	
	protected UniformUploaderColor[] line_mulColor=new UniformUploaderColor[20];
	protected UniformUploaderF2      line_middle[]=new UniformUploaderF2[20], line_end[]=new UniformUploaderF2[20];
	protected UniformUploaderF1 line_widthPos1[]=new UniformUploaderF1[20], line_widthPos2[]=new UniformUploaderF1[20], line_totalLength[]=new UniformUploaderF1[20], lineArraySize;
	...
	private static class CricleFX implements BaseFX{
		...
		private int    id;
		float distanceScale, sizeP, sizeN, radius;
	...
	private static class LineFX implements BaseFX{
		...
		public LineFX(Vec3M worldPos1, Vec3M worldPos2, float width, ColorM mulColor){
			...
			PairM<Vec2FM,Float> o=convertWorldToScreenPos(worldPos1);
		...
		public void upload(){
			int i=getArrayId();
			Vec2FM siz=new Vec2FM(Display.getWidth()/2F, Display.getHeight()/2F), mid=start.add(end).div(2).mul(siz), en=end.mul(siz);
