...
public class EntityPenguinRenderer extends RenderLiving<EntityPenguin>{
	...
	private static class MainModel extends ModelBase{
		...
		public void render(Entity e, float limbSwing, float limbSwingAmount, float ageInTicks, float headYaw, float headPitch, float scale){
			...
			float 
				bodyRotPrecent=PartialTicksUtil.calculate(penguin.animator.prevSlidingTransition, penguin.animator.slidingTransition),
				swimming=PartialTicksUtil.calculate(penguin.animator.prevSwimmingMul, penguin.animator.swimmingMul),
				bodyRot90=bodyRotPrecent*90,
				swimRot=bodyRotPrecent*PartialTicksUtil.calculate(penguin.animator.prevSwimRot, penguin.animator.swimRot),
				rightArm1X=0,
				rightArm1Z=0,
				rightArm2Z=0,
				leftArm1X=0,
			...
			if(swimming!=0){
				float 
					pos=((limbSwing+limbSwingAmount)/6)%1,
					mul=(float)(swimming*Math.PI),
					arm1=swimmming.get(0,pos)*mul,
					arm2=swimmming.get(1,pos)*mul;
			...
			float 
				cos1=MathHelper.cos(ageInTicks*0.09F)*0.03F+0.03F,
				cos2=MathHelper.cos(ageInTicks*0.015F)*4F,
				sin1=MathHelper.sin(ageInTicks*0.067F)*0.03F,
				sin2=MathHelper.sin(ageInTicks*0.04F)*0.03F,
				sin3=MathHelper.sin(ageInTicks*0.01F)*4F;
			int speed=30;
